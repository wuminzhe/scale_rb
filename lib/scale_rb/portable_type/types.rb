...
module ScaleRb
  module PortableType
    ...
    class VariantType
      ...
      # Helper method to create an Option type
      # % self.option :: Ti -> VariantType
      def self.option(inner_type)
        new(
          [
            SimpleVariant.new(:None, 0),
            TupleVariant.new(:Some, 1, [inner_type])
          ]
        )
      end

      # Helper method to create a Result type
      # % self.result :: Ti -> Ti -> VariantType
      def self.result(ok_type, err_type)
        new(
          [
            TupleVariant.new(:Ok, 0, [ok_type]),
            TupleVariant.new(:Err, 1, [err_type])
          ]
        )
      end
    end
