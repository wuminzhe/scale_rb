module ScaleRb
  module ClientHelper
    class << self
      def get_metadata(client, block_hash)
        dir = ENV['SCALE_RB_METADATA_DIR'] || File.join(Dir.pwd, 'metadata')

        get_metadata_by_block_hash(client, dir, block_hash)
      end


      # get storage at block_hash
      def get_storage(client, block_hash, pallet_name, storage_name, key_part1: nil, key_part2: nil)
        metadata = get_metadata(client, block_hash)
      ...
      def query_storage_at(client, block_hash, storage_keys, type_id, default, registry)
        result = client.state_queryStorageAt(storage_keys, block_hash)
        result.map do |item|
      ...
      def get_storage_keys_by_partial_key(client, block_hash, partial_storage_key, start_key = nil)
        storage_keys = client.state_getKeysPaged(partial_storage_key, 1000, start_key, block_hash)
        if storage_keys.length == 1000
          storage_keys + get_storage_keys_by_partial_key(client, block_hash, partial_storage_key, storage_keys.last)
        else
          storage_keys
        end
      end

      def get_storages_by_partial_key(client, block_hash, partial_storage_key, type_id_of_value, default, registry)
        storage_keys = get_storage_keys_by_partial_key(client, block_hash, partial_storage_key, partial_storage_key)
      ...
      def get_storage1(client, block_hash, pallet_name, item_name, key, value, registry)
        if key
          ...
          data = client.state_getStorage(storage_key, block_hash)
          StorageHelper.decode_storage(data, value[:type], value[:modifier] == 'Optional', value[:fallback], registry)
        end
      end

      def get_storage2(client, block_hash, pallet_name, item_name, value_of_key, metadata)
        ...
        get_storage1(client, block_hash, pallet_name, item_name, key, value, registry)
      end
